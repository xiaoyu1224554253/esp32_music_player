...
{
    // ES3C28P 无 EC06 编码器 (PIN_EC06_A/B = -1)，真实读取被禁用。
    // 触摸层通过 keys_inject_encoder_step() 模拟旋钮步进。
    if (PIN_EC06_A < 0 || PIN_EC06_B < 0) {
        return s_enc_last;  // 保持状态，不产生 quadrature 边沿
    }
...
{
    // ES3C28P：仅 BOOT 键(GPIO0) 为真实 GPIO，其余导航键全部禁用，
    // 由 FT6336 触摸层通过 keys_inject_event() 注入 key_event_t。
    if (pin < 0) {
        return HIGH;  // 禁用引脚恒为未按下
    }
...
// ============================================================
// FT6336 触摸注入接口 (ES3C28P)
// 触摸层将滑动/点击手势映射为 key_event_t，复用与物理按键相同的分发逻辑。
// ============================================================
void keys_inject_event(key_event_t evt)
{
  if (player_list_select_is_active()) {
    list_select_key_and_refresh(evt);
    return;
  }

  if (quick_menu_is_active()) {
    switch (evt) {
      case KEY_NEXT_SHORT:      quick_menu_key_and_refresh(QuickMenuKey::Down); break;
      case KEY_PREV_SHORT:      quick_menu_key_and_refresh(QuickMenuKey::Up); break;
      case KEY_PLAY_SHORT:      quick_menu_key_and_refresh(QuickMenuKey::Confirm); break;
      case KEY_MODE_SHORT:      quick_menu_key_and_refresh(QuickMenuKey::Back); break;
      case KEY_MODE_LONG:       quick_menu_key_and_refresh(QuickMenuKey::Exit); break;
      case KEY_PAGE_DOWN_SHORT: quick_menu_key_and_refresh(QuickMenuKey::PageDown); break;
      case KEY_PAGE_UP_SHORT:   quick_menu_key_and_refresh(QuickMenuKey::PageUp); break;
      default: break;
    }
    return;
  }
  ...
  // 正常播放模式
  switch (evt) {
    case KEY_PLAY_SHORT:  player_toggle_play(); break;
    case KEY_NEXT_SHORT:  player_next_track(); break;
    case KEY_PREV_SHORT:  player_prev_track(); break;
    case KEY_MODE_SHORT:  volume_fast_mode_toggle(); break;
    case KEY_MODE_LONG:   enter_quick_menu_from_player(); break;
    default: break;
  }
}